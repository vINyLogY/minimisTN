...
def ref():
    ...
    exp.root.is_normalized = True
    exp.settings(cmf_steps=10, ode_method='RK45')
    z1 = list(exp.autocorr(
        steps=STEPS, ode_inter=ODE_INTER, fast=True, split=False))
    np.save('./tmp/ref_sho', z1)
    return
...
def main(split=False):
    ...
    solver.settings(cmf_steps=1, ode_method='RK23', ps_method='s')
    start = time()
    z = list(solver.autocorr(steps=STEPS, ode_inter=ODE_INTER, fast=True,
                             split=split,))
    end = time()
    print(end - start)
    np.save('./data/exp_sho_{}'.format('split' if split else 'origin'), z)
