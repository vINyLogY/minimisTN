...
def plot():
    ...
    t2, a2 = zip(*np.load('./tmp/exp_sho_split.npy'))
    t3, a3 = zip(*np.load('./tmp/exp_sho_origin.npy'))
    with figure():
        plt.plot(t1, np.abs(a1)**2, '-', label='MCTDH')
        plt.plot(t3, np.abs(a3)**2, '--', label='ML-MCTDH')
        plt.plot(t2, np.abs(a2)**2, '--', label='ML-MCTDH-PS')
        plt.legend(loc='best')
        plt.xlabel(r'$t$ (a. u.)')
        plt.ylabel(r'$P$')
        plt.show()
