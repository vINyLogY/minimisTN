...
def main():
    ...
    for t in root.visitor():
        ...
        if int(t.name) == 0:
            ...
            array = np.zeros((n_1, n_1 ** 2))
            for n, v_i in zip(triangular(n_1), array):
                v_i[n] = 1.
            array = np.reshape(array, (n_1, n_1, n_1))
            array = Tensor.partial_product(array, 1, array_i)
            array = Tensor.partial_product(array, 2, array_i)
        else:
            array = None
            leaves.append(t)
        t.set_array(array)
    ...
    start = time()
    t2, a2 = zip(*
        solver.autocorr(steps=100, ode_inter=0.01, fast=True, split=True))
    end = time()
    print(end - start)
